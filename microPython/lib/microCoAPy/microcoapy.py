...
import microcoapy.reading as reading
...
class Coap:
    ...
    def start(self, port=_COAP_DEFAULT_PORT, period=0):
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

        # https://stackoverflow.com/questions/2351465/socket-shutdown-and-rebind-how-to-avoid-long-wait
    ...
    def loop(self, blocking=True):
        ...
        if (remoteAddress is not None):
            #print("Buffer len: " + str(len(buffer)) + ", Remote Address: " + remoteAddress[0])
            self.stats[self.period].attack += 1
    ...
    def poll(self, timeoutMs=-1, pollPeriodMs=100):
        start_time = time.ticks_ms()
        while not self.loop(False) and (abs(time.ticks_diff(time.ticks_ms(), start_time)) < timeoutMs):
